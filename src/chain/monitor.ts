...
export class ChainMonitor {
  private provider = new WebSocketProvider(config.chain.rpc.replace('https://', 'wss://'));
  private breaker = new CircuitBreaker(5, 20000);
  ...
  start(): void {
    ...
    usdc.on('Transfer', (from: string, to: string, value: bigint, ev: any) => {
      if (!this.breaker.canExecute()) return;
      bus.emit('chain:transfer', { token: 'USDC', from, to, value: formatUnits(value, 6), txHash: ev.log.transactionHash, ts: Date.now() });
    });

    ctf.on('Transfer', (from: string, to: string, value: bigint, ev: any) => {
      if (!this.breaker.canExecute()) return;
      bus.emit('chain:transfer', { token: 'CTF', from, to, value: value.toString(), txHash: ev.log.transactionHash, ts: Date.now() });
    });

    ctf.on('ConditionResolution', (conditionId: string, _q: string, payoutNumerators: bigint[]) => {
